class Solution {
    bool findWord(vector<vector<char>>& board, int y, int x, int index, string &word, vector<vector<int>> seen){
        ...
        // TL, valid, not seen, and character matches
        if(y-1 >= 0 && x-1 >= 0 && seen[y-1][x-1] != 1 && word[index] == board[y-1][x-1]){
            seen[y][x] = 1;
            index++;
            bool r = findWord(board, y-1, x-1, index, word, seen);
            if(r){
                return true;
            }
            index--;
        ...
        // TR
        if(y-1 >= 0 && x+1 < board[0].size()  && seen[y-1][x+1] != 1 && word[index] == board[y-1][x+1]){
            seen[y][x] = 1;
            index++;
            bool r = findWord(board, y-1, x+1, index, word, seen);
            if(r){
        ...
        // BL
        if(y+1 < board.size() && x-1 >= 0 && seen[y+1][x-1] != 1 && word[index] == board[y+1][x-1]){
            seen[y][x] = 1;
            index++;
            bool r = findWord(board, y+1, x-1, index, word, seen);
            if(r){
        ...
        // BR
        if(y+1 < board.size() && x+1 < board[0].size() && seen[y+1][x+1] != 1 && word[index] == board[y+1][x+1]){
            seen[y][x] = 1;
            index++;
            bool r = findWord(board, y+1, x+1, index, word, seen);
            if(r){
